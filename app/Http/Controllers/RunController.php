...
class RunController extends Controller
{
    ...
    {
        $user = new User();
        list($name,$mobile) = $this ->getName();
        $user->username = $this ->getNickName();
        $user->password = md5('xiaoyuan123456');
        $user->phone = $mobile;
        $user->sex = rand(1,2);
        $user->head_image = 'images/user/user_default.jpg';
        list($email,$birth) = $this->getEmailsex($user->sex);
        $user->email = $email;
        $user->birth = $birth;
        $user->save();

        $uid = $user->id;
        UserInfo::firstOrCreate(['uid'=>$uid,'user_name'=>$name]);
    }
    ...
    private function getEmailsex($sex=1)
    {
        ...
        return array($name.$birthuse.'@'.$wei[array_rand($wei)],$birth);
    }
